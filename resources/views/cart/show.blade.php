      ...
      <div class="flex space-x-6">
        ...
        <div class="min-w-[35%] flex flex-col justify-start mt-6">
          <table class="table-auto border border-gray-300 text-sm mb-4">
            <thead class="bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-200">
              <tr><th colspan="2" class="px-4 py-2 text-left">Resumen</th></tr>
            </thead>
            <tbody>
          ...
          <div class="space-y-2">
            <form method="POST" action="{{ route('cart.destroy') }}">
              @csrf
              @method('DELETE')
              <button type="submit" class="px-4 py-2 bg-red-500 text-white rounded w-full">Vaciar Carrito</button>
            </form>
            <a href="#" class="block w-full text-center bg-green-600 hover:bg-green-700 text-white py-2 rounded">Proceder al Pago</a>
          </div>
        </div>
      </div>
    @else
